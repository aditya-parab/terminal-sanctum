...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Render first frame immediately so the screen is never blank.
    terminal.draw(|f| ui(f, &mut app))?;

    let (tx, rx) = mpsc::channel();
    let mut watcher = RecommendedWatcher::new(tx, Config::default())?;
    let cwd_head = PathBuf::from(".git/logs/HEAD");
    if cwd_head.exists() {
        watcher.watch(&cwd_head, RecursiveMode::NonRecursive)?;
    }
    for entry in std::fs::read_dir(".")?.flatten() {
        let head_log = entry.path().join(".git/logs/HEAD");
        if head_log.exists() {
            watcher.watch(&head_log, RecursiveMode::NonRecursive)?;
        }
    }
                ...
                InputMode::SelectingProfile => match key.code {
                    ...
                    KeyCode::Enter => {
                        if let Some(i) = app.profile_list_state.selected()
                            && let Some(name) = app.profile_list.get(i)
                            && let Ok(avatar) = load_avatar(name)
                        {
                            app.avatar = avatar;
                            let cwd =
                            ...
                            let abs_path = cwd
                                ...
                                .to_string();
                            app.avatar
                                .add_log(format!("Soul Incarnated. Workspace: {abs_path}"));
                            app.avatar.contributions.clear();
                            let (scan_tx, scan_rx) = mpsc::channel();
                            app.scan_rx = Some(scan_rx);
                            thread::spawn(move || {
                                let history = Avatar::perform_passive_scan(".");
                                scan_tx.send(history).ok();
                            });
